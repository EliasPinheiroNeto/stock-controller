...
export default class ItemController extends Controller {
    protected assignRoutes(): void {
        ...
        this.router.get(
            "/items/standalone",
            this.getAllWithNoCategory.bind(this),
        );
    ...
    private async create(req: Request, res: Response) {
        ...
        try {
            const data = RequestService.validateAuthHeader(
                req.headers.authorization,
            );

            const item = await itemService.insert(
                data.userId,
                body,
                data.employeeId,
            );
        ...
        try {
            ...
            if (data.userId != item.user_id) {
                throw new ApplicationError(
                    "Autorization error on update item",
                    {
                        status: 401,
    ...
    private async delete(req: Request, res: Response) {
        ...
        try {
            ...
            if (data.userId != item.user_id) {
                throw new ApplicationError(
                    "Autorization error on delete item",
                    {
                        status: 401,
                        errorCode: "UNAUTHORIZED",
                        message:
